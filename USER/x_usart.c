...
// ========== 视觉通信专用接收缓冲区 ==========
#define VISION_BUF_SIZE 128
static char vision_rx_buf[VISION_BUF_SIZE];
static u16 vision_rx_index = 0;
static u8 vision_frame_ready = 0;  // 一帧完整数据就绪标志

// 查询是否有完整帧到达
u8 USART2_Vision_GetRxFlag(void) {
    return vision_frame_ready;
}

// 读取视觉数据字符串
void USART2_Vision_ReadString(char *buf) {
    strcpy(buf, vision_rx_buf);
    vision_frame_ready = 0;      // 清除标志
    vision_rx_index = 0;         // 重置索引
    memset(vision_rx_buf, 0, VISION_BUF_SIZE);
}

// 清除视觉接收缓冲区（出错时调用）
void USART2_Vision_ClearRxBuf(void) {
    vision_rx_index = 0;
...
void USART2_IRQHandler(void) {
    ...
    if (USART_GetFlagStatus(USART2, USART_IT_RXNE) == SET) {
        ...
        // 忽略缓冲区溢出
        if (vision_rx_index >= VISION_BUF_SIZE - 1) {
            vision_rx_index = 0;   // 丢弃
            return;
        }
        ...
        // 收到换行符，一帧结束
        if (ch == '\n') {
            vision_rx_buf[vision_rx_index] = '\0';  // 字符串结束
            // 仅当帧以 'V' 开头时才认为有效
            if (vision_rx_buf[0] == 'V') {
                vision_frame_ready = 1;
            }
            vision_rx_index = 0;   // 无论有效无效，都重置索引
        }
    }
}
...
int fputc(int ch, FILE *f)
{
	...
	tb_usart1_send_byte(ch);			//将printf的底层重定向到自己的发送字节函数
	return ch;
}
